...
>  깃 연동 안되면 git init 했는지 확인하고 이름 이메일
...
**일괄 실행과 지연 실행의 알림창 결과가 동일한 이유**  
버튼을 클릭하여 `setNumber(number + 5)`를 호출하더라도, 해당 함수 스코프 내의 `number` 변수 값은 즉시 변경되지 않는다.  
따라서 `setNumber` 직후에 실행되는 `alert(number)`는 현재 렌더링 시점의 값인 0을 출력한다.  

타이머(`setTimeout`)를 사용하더라도 1초 뒤에 뜨는 알림창에 똑같이 0이 출력되는데, 이는 자바스크립트의 클로저의 특성 때문임    
타이머 내부의 콜백 함수는 자신이 생성된 시점의 환경을 기억한다. 즉, 1초 동안 React가 상태를 업데이트하고 화면을 다시 그렸더라도,  
타이머 안의 `alert`는 과거 시점(버튼을 누른 시점)에 고정된 `number`의 스냅샷인 0을 참조하게 된다.  

**해결 방법 : 계산된 값을 일반 변수에 할당**
상태 업데이트 지연과 상관없이 의도한 값을 즉시 사용하려면, 계산된 값을 먼저 일반 변수에 할당하고 이를 활용해야 한다.

```jsx
function handleTimer () {
  const nextNumber = number + 5; // 변경할 값을 미리 변수에 할당
  setNumber(nextNumber);         // 반영된 변수를 상태 업데이트에 사용

function handleIncrease3 () { // 대기열에 있는 최신 값을 받아와 계산
  setNumber(n => n + 1); // 0 + 1
  setNumber(n => n + 1); // 1 + 1
  setNumber(n => n + 1); // 2 + 1
}
...
> [!NOTE]
> **변수 할당 방식 vs 업데이터 함수 방식**  
> **변수 할당** (const next = number + 5) : 변경될 값을 같은 함수 내에서 즉시 사용해야 할 때 사용한다.   
> **업데이터 함수** (setNumber(n => n + 1)) : 이전 상태에 의존하여 연속적으로 상태를 업데이트해야 할 때 사용한다. 
...
```
추가로 코드에 적힌 것처럼 5번째 이후로 렌더링 하려하면 UI가 출력되지 않는데 이는 오류 발생시 UI를 없애버리기 때문임  

> [!warning]
> **Hook 사용시 주의 할 점**  
> Hook을 사용하기 위해서는 일반 모듈과 마찬가지로 import 해서 사용해야 함  
> Hook은 컴포넌트의 최상위 수준 또는 사용자 정의 Hook에서만 호출할 수 있다.  
> 조건문, 반복문 또는 기타 중첩 함수 내부에서는 Hook을 호출할 수 없습니다.  
